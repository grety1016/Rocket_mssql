    ...
    T: Topic
{
    ...
    fn deref(&self) -> &Self::Target { self.pack.as_ref::<T>() }
}
...
where
    ...
    T::Message: Debug
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { Debug::fmt(self.deref().deref(), f) }
}
...
where
    ...
    T::Message: Display
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { Display::fmt(self.deref().deref(), f) }
}
...
impl PublishEvent {
    ...
    /// 解包
    fn unpack(self) -> (TypeId, MessagePack) { (self.topic_id, MessagePack(self.msg.into())) }
}

impl Debug for PublishEvent {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { write!(f, "publish pack") }
}
